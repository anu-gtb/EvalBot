...
if st.session_state.table:
   max_marks=st.data_editor(max_marks)
   
if button2:
   for i in range(0,len(max_marks['Student File No.'])):
         marks['Marks Obtained'][i]=(predict_score_and_accuracy(dataframe(answers,answers_by_student))[1][i]*max_marks['Maximum Marks'][i]).round(0)
   ...
   st.table(max_marks)  
   st.write('Total Marks Obtained : ',max_marks['Marks Obtained'].sum())